"""
Task 1: Helmholtz Equation with k=4
Training script - focuses on training and saving results
Visualization is done separately in task1_visualize.py
"""
...
def sample_boundary(N_per_side, device):
    """Sample boundary points on the four sides."""
...
def train(net, device, epochs=30000, N_interior=8000, N_per_side=500, lr=0.001):
    """Training loop."""
    ...
    pbar = tqdm(range(epochs), desc='Training Task 1')
    ...
    print(f"\nTraining Complete! Best Loss: {best_loss:.6e}")
    return loss_history, best_loss

def evaluate(net, device, grid_size=100):
    """Evaluate the trained model on a test grid."""
    # Create test grid
...
def main():
    ...
    print(f"Task 1: Helmholtz Equation with k={k}")
    print("="*80)
    ...
    # Evaluate
    print("\nEvaluating model...")
    metrics = evaluate(net, device, grid_size=100)
    ...
    with open('results/data/training_info.json', 'w') as f:
        json.dump(training_info, f, indent=4)
    
    print("\n" + "="*80)
    print("Training and evaluation complete!")
    print("Results saved to results/data/")
    print("Run task1_visualize.py to generate paper figures.")
    print("="*80)
