...
def compute_loss(net, xy_i, xy_b, lambda_pde=1.0, lambda_bc=100.0):
    """
    ...
    delta_u = du_xx + du_yy
    pde_res = -delta_u - (k**2) * u_i.squeeze() - q(xy_i[:, 0], xy_i[:, 1])
...
def train(net, device, epochs=80000, N_interior=8000, N_per_side=500, lr=0.001):
    """
    ...
    loss_history = {'epoch': [], 'total_loss': [], 'pde_loss': [], 'bc_loss': []}
    best_loss = float('inf')
    ...
    for epoch in pbar:
        ...
        loss, L_pde, L_bc = compute_loss(net, xy_i, xy_b)
        ...
        if current_loss < best_loss:
            ...
            torch.save(net.state_dict(), 'results/models/best_model.pth')
        ...
        if (epoch + 1) % 2000 == 0:
            print(f'\n{"="*80}')
            print(f'Epoch {epoch+1}: Loss={current_loss:.6e}, Best={best_loss:.6e}')
            print(f'{"="*80}\n')
    
    # 保存损失历史
    pd.DataFrame(loss_history).to_csv('results/data/loss_history.csv', index=False)
    
    print(f"\nFinish! Best Loss: {best_loss:.6e}")
    
    return loss_history

# Evaluation and visualization
def evaluate_and_plot(net, device, loss_history):
    """
    ...
    # 保存评估数据
    np.savez('results/data/eval_data.npz', X=X, Y=Y, u_pred=u_pred, u_true=u_true_val, error=error)
    ...
    # Loss convergence
    axs[1, 1].plot(loss_history['total_loss'])
    ...
    plt.tight_layout()
    plt.savefig('results/figures/helmholtz_pinn_results.png')
    plt.savefig('results/data/results_plot.png')
...
def main():
    ...
    net = Net(layers=[2, 100, 100, 100, 100, 1], omega_0=30.0)  # SIREN with omega_0
    net.to(device)
    
    # Train
    loss_history = train(net, device, epochs=30000, N_interior=8000, N_per_side=500, lr=0.001)
    
    # Load best model (saved during training)
    print("\nLoad Best Model...")
    net.load_state_dict(torch.load('results/models/best_model.pth'))
    
    # Evaluate and plot
    evaluate_and_plot(net, device, loss_history)
